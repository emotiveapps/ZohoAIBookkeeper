...
**Receipts**: forward any receipt email to your configured Microsoft 365 mailbox and `receipts sync` parses it with Claude, archives the original + metadata in your receipt archive (`receipts.archive_path` in config.json — a visible, backed-up folder such as a synced OneDrive finance folder), and attaches it to the matching Zoho expense — automatically when the match is unambiguous, held and retried when the expense hasn't hit the bank feed yet. Every processed email is then **moved (never deleted) into `Bookkeeper/{Matched, Pending, Needs Review, Not a Receipt}`** in the mailbox, so Outlook shows each message's outcome and the Inbox stays an empty to-process queue. Pending emails promote to Matched automatically once their expense appears. Configure mailboxes in the `receipts` section of config.json; the Entra app needs delegated `Mail.ReadWrite.Shared`.
