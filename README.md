...
**Receipts**: forward any receipt email to your configured Microsoft 365 mailbox and `receipts sync` parses it with Claude, archives the original + metadata in the cloud-canonical receipt archive (a OneDrive folder, `receipts.archive_folder_path` in config.json, synced through the Graph API with a local cache — every device sees the same archive), and attaches it to the matching Zoho expense — automatically when the match is unambiguous, held and retried when the expense hasn't hit the bank feed yet. Every processed email is then **moved (never deleted) into `Bookkeeper/{Matched, Pending, Needs Review, Not a Receipt}`** in the mailbox, so Outlook shows each message's outcome and the Inbox stays an empty to-process queue. Pending emails promote to Matched automatically once their expense appears. Configure mailboxes in the `receipts` section of config.json; the Entra app needs delegated `Mail.ReadWrite.Shared`.

A OneDrive folder can serve as a second receipts inbox (`receipts.onedrive.folder_path` in config.json): `receipts sync` sweeps it through the Microsoft Graph API — cloud-side, so it works no matter which machine runs the sync — parses every PDF/image the same way as email receipts, and moves processed files into `{Matched, Pending, Needs Review, Not a Receipt}` subfolders, preserving your original folder structure beneath them. Drop a receipt into the folder from any device (including the OneDrive app on your phone) and the next sync files it. Non-receipt files (scripts, CSVs, zips) are never touched, and nothing is ever deleted. Requires delegated `Files.ReadWrite` on the Entra app.
