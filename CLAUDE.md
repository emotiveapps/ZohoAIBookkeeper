...
- **App state**: `@MainActor @Observable` classes (Observation framework; app targets iOS 26+ and uses Liquid Glass for floating controls — action bar, badges), not ObservableObject. The old `ViewModels/` layer is gone — don't recreate it; screen logic lives in `ReviewSession`/`Workspace`.
